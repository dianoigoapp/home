...
const INDEX_KEY = 'discipular.offlineIndex.v1';
...
export async function downloadCourseOffline(courseId: string, lessonIds: string[]) {
  ...
  for (const lessonId of lessonIds) {
    ...
    if (isNative) {
      ...
      localStorage.setItem(`discipular.lesson.${courseId}.${lessonId}`, md);
    }
  }
...
export async function getLessonOffline(courseId: string, lessonId: string): Promise<string | null> {
  ...
  try {
    ...
    const raw = localStorage.getItem(`discipular.lesson.${courseId}.${lessonId}`);
    return raw ?? null;
  } catch {
    return null;
  }
